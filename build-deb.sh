...
# Multi-arch mode: ./build-deb.sh --all
if [ "${1-}" = "--all" ]; then
  targets=( "amd64:linux-x64" "arm64:linux-arm64" "arm:linux-arm" )
  for t in "${targets[@]}"; do
    ARCH="${t%%:*}"
    RUNTIME="${t##*:}"
    OUTDIR="publish/${RUNTIME}"
    STAGE="${PKG_NAME}_${VERSION}_${ARCH}"
    echo "\n=== Building ${PKG_NAME} ${VERSION} for ${ARCH}/${RUNTIME} ==="
    build_package
  done
  exit 0
fi

# Default: single build with variables above
build_package
